        ...
        callback: async (label, args, shellmin) => {
            ...
            if (args.length < 1) {
                ...
                if (shellmin.hasCommand(args[0])) {
                    ...
                    clog_f("Command \"" + args[0] + "\" not found.");
            }
        }
    },
};
...
class Environment {
        ...
        toConvert_parts.forEach((toConvert_part) => {
            if (toConvert_part instanceof Object && !(toConvert_part instanceof Array)) {
                Object.keys(toConvert_part).forEach((key) => {
                    typeof toConvert_part[key] == "string" ? __classPrivateFieldGet(this, _dictionary)[key] = nodtilus_1.default.json.types.check.instantiate(toConvert_part[key]) : null;
                });
            }
        ...
        for (let i = 0; i < toConvert.length; ++i) {
            ...
            if (typeof p == "string") {
                ...
                if (p.startsWith("-")) {
                    ...
                    p = arg_key = p.startsWith("-") ? p.substring(1, p.length) : p;
                }
                let p2 = p.split("=");
                ...
                arg_value = f_arg_v_r.content ? nodtilus_1.default.json.types.check.instantiate(f_arg_v_r.content) : true;
    ...
    remove(key, value) {
        if (typeof key == "string") {
            ...
            __classPrivateFieldSet(this, _array_dictionary, nodtilus_1.default.arrays.removeFromArray(__classPrivateFieldGet(this, _array_dictionary), key));
        }
    }
    has(key) {
        return this.get(key);
...
class Shellmin extends events_1.EventEmitter {
        ...
        _live_stdin_data_callback.set(this, async (d, key) => {
            if (!__classPrivateFieldGet(this, _closed)) {
                if (vkb_1.VirtualKeyBoard.is(key, vkb_1.VirtualKeyBoard.KEYS.INTERACTIONS.TERMINAL.RETURN)) {
                    ...
                    __classPrivateFieldSet(this, _old_line_size, __classPrivateFieldGet(this, _line).length);
    ...
    //
    async first(options = { bindConsole: true }) {
        options = options || {};
        options.bindConsole = options.bindConsole != undefined ? options.bindConsole : true;
        options.bindConsole == true ? this.bindConsole() : null;
        //
        let cmd_info = {
            label: null,
            args: []
        ...
        //
        options.bindConsole == true ? this.unbindConsole() : null;
    }
    live() {
    ...
    unregisterCommand(command) {
        ...
        if (_command) {
            __classPrivateFieldSet(this, _commands, nodtilus_1.default.arrays.removeFromArray(__classPrivateFieldGet(this, _commands), __classPrivateFieldGet(this, _commands).indexOf(_command)));
        }
        return this;
    }
    getCommand(command) {
