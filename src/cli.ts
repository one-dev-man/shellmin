...
import nodtilus, { NODTILUS_JSON_TYPES } from "nodtilus";
    ...
    help: {
        ...
        callback: async (label: string, args: Array<any>, cli: CLI) => {
            ...
            if(args.length < 1) {
                ...
                else clog_f("Command \""+args[0]+"\" not found.");
            }
        }
    },
    // history: {
...
export class Environment {
    ...
    #array_dictionary = new Array<NODTILUS_JSON_TYPES>();
    #dictionary: {[key: string]: NODTILUS_JSON_TYPES} = {};
    ...
    reinit(...toConvert_parts: Array<any>) {
        ...
        toConvert_parts.forEach((toConvert_part) => {
            if(toConvert_part instanceof Object && !(toConvert_part instanceof Array)) {
                Object.keys(toConvert_part).forEach((key: string) => {
                    typeof toConvert_part[key] == "string" ? this.#dictionary[key] = nodtilus.json.types.check.instantiate(toConvert_part[key]) : null;
                });
            }
        ...
        for(let i = 0; i < toConvert.length; ++i) {
            ...
            if(typeof p == "string") {
                ...
                arg_value = f_arg_v_r.content ? nodtilus.json.types.check.instantiate(f_arg_v_r.content) : true;
    ...
    remove(key: string | number, value: any) {
        if(typeof key == "string") {
            ...
            this.#array_dictionary = nodtilus.arrays.removeFromArray(this.#array_dictionary, key);
        }
    }
...
export class CLI extends EventEmitter {
    ...
    async first(options: { bindConsole: boolean } = { bindConsole: true }) {
        options = options || {};
    ...
    unregisterCommand(command: Command | string) {
        ...
        if(_command) {
            this.#commands = nodtilus.arrays.removeFromArray(this.#commands, this.#commands.indexOf(_command));
        }
    ...
    async runCommand(label: string, ...args: Array<any>): Promise<any> {
        try {
